...
\begin{equation}
    ...
    \end{pmatrix}.
\end{equation}
The goal is to provide the universal construction for these extra matrix element $f_1, \partial_1, f_0, \partial_0$.
...
Based on these two sets, we can define the projection to the sets. For any ordered subset $S=(s_0,\dots,s_{|S|-1})$, define the projection $\pi_S \in \mathbb{F}_2^{|S|\times N}$ such that
  ...
  Therefore we can construct a matrix how $Z$ stabilizers overlap with the logical $\bar{X}$, the vertex--edge incidence  $(\partial_1)_{i,j} =  (\pi_{V_0} H_Z^T \pi_{C_0}^{T})_{i,j} = (H_Z)_{c_j, v_i}$, with $\partial_1 \in \mathbb{F}_2^{|V_0|\times|C_0|}$. We take $\partial_1$ in this vertex--edge orientation, so it enters $\tilde H_X$ untransposed; the cited arXiv:2410.02753 Eq.~(62) writes the edge--vertex $\partial_1$ and hence the transpose $\partial_1^T$. Besides, since the new introduced X stabilizer also should overlap with the original qubit $Q$, the data attachment is $f_1^T = \pi_{V_0} \in \mathbb{F}_2^{|V_0|\times n}$ (i.e.\ $f_1 = \pi_{V_0}^T$). From here we find that we have $|V_0|$ new $S_X'$ ancilla stabilizer and $C_0$ new $Q'$ anciall qubits.

  Since we've introduced new ancilla qubits $Q'$, the original $S_Z$ should has more support $f_0 = \pi_{C_0}^T \in \mathbb{F}_2^{m_Z\times  |C_0|}$. Finally, to perform the gauge fixing, we have to introduce extra $S_Z'$ stabilizer $\partial_0$ such that $\partial_1 \partial_0^T = 0$. This is because we need to fix the extra logical qubits in the introducing of the ancilla qubits. To completely fix the gauge we have $\operatorname{row}( \partial_0) =  \ker \partial_1 \in \mathbb{F}_2^{|C_0|}$, i.e.\ $\partial_0$ spans the full subspace of ancilla $Z$-operators commuting with the new $X$-stabilizers (this gauge block is written $G$ in arXiv:2410.02753 Eq.~(62) and $\partial_0$ in Eq.~(68); we use $\partial_0 = \ker \partial_1$ throughout). Without introducing $\partial_0$, the logical operator dim is $n+|C_0|-m_X-m_Z-|V_0|$. After introducing the $\partial_0$ with gauge fixing, the dimension is $n+|C_0|-m_X-m_Z-|V_0|-r$, where $r= \dim \operatorname{row} \partial_0=|C_0|-|V_0|+\dim \ker(\partial_1^T)$. Therefore the final merged code logical dimension is $k-\dim \ker(\partial_1^T)$. In the low-rate surgery we have $\dim \ker(\partial_1^T)=1$, because $\ker(\partial_1^T) \in \mathbb{F}_2^{|V_0|}$ means the linear independence of the stabilizer $S_X'$ on the ancilla qubits. 
...
The new $X$ stabilizer is
  \begin{equation}
 (S_X')_i = \prod_{l=0}^{n-1} X_l^{(f_1^T)_{i,l}} \prod_{k=0}^{|C_0|-1} X_{\kappa_k}^{(\partial_1)_{i,k}}, \quad
  i=0,\dots,|V_0|-1.
  \end{equation}

The exponent on each ancilla $\kappa_k$ in $\prod_i (S_X')_i$ is
  \begin{equation}
  \sum_{i=0}^{|V_0|-1}(\partial_1)_{i,k}
...
\subsection{Construction}
  ...
  \item[\textsc{SkipTree}.]     By Swaroop et al.\ Thm.~7, the input is the  the cellulated port subgraph $(\partial_1^{s,\mathrm{aug}})^T$ and the output is a $(3,2)$-sparse
  matrix $T_s$ and permutation matrix $P_{\sigma_s}$ with
   \begin{equation}
  \underbrace{T_s}_{(w-1)\times m^s_{\mathrm{aug}}}
  \Bigg(\underbrace{(\partial_1^{s,\mathrm{aug}})^T}_{m^s_{\mathrm{aug}}\times|V_0^s|}
  ...
  H_X^l & 0 & 0 & 0 & 0 \\
  0 & H_X^r & 0 & 0 & 0 \\
  f_1^{l,T} & 0 & \partial_1^{l,\mathrm{aug}} & 0 & \pi_{\mathcal{P}_l}^T P_{\sigma_l} \\
  0 & f_1^{r,T} & 0 & \partial_1^{r,\mathrm{aug}} & \pi_{\mathcal{P}_r}^T P_{\sigma_r}
  \end{pmatrix},
  \end{equation}
  \begin{equation}
...
Expand $\tilde H_Z^{\mathrm{joint}} (\tilde H_X^{\mathrm{joint}})^T$ as a
  ...
  \begin{equation}
  T_s (\partial_1^{s,\mathrm{aug}})^T \;+\; H_R\, P_{\sigma_s}^T \pi_{\mathcal{P}_s}.
  \end{equation}
  The SkipTree identity is
  \begin{equation}
  T_s (\partial_1^{s,\mathrm{aug}})^T \pi_{\mathcal{P}_s}^T = H_R\, P_{\sigma_s}^T.
  \end{equation}
  Because
  $T_s (\partial_1^{s,\mathrm{aug}})^T$ is already zero on non-port columns of
